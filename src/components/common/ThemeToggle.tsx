...
export const ThemeToggle: React.FC<ThemeToggleProps> = ({ className = '' }) => {
  ...
  return (
    <button
      type="button"
      onClick={toggleTheme}
      className={`w-9 h-9 inline-flex items-center justify-center rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#1e293b] text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all cursor-pointer ${className}`}
